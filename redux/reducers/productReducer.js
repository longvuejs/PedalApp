export const UPDATE_ISLOVE_STATUS = 'UPDATE_ISLOVE_STATUS';
const initialState = {
  ListPedal: [
    {
      id: 1,
...
export default function actionProductReducer(state = initialState, action) {
  // console.log(action.payload);
  switch (action.type) {
    case 'ListPedal/updateLove':
      return {
        ...state,
        ListPedal: [
          ...state.ListPedal.map(item =>
            item?.id === action?.payload?.id
              ? {
                  ...item,
                  isLove: action.payload.isLove,
                }
              : {
                  ...item,
                  isLove: item.isLove,
          ...
          ...state.ListPedal.map(item =>
            {
              return {
                ...item,
                isLove: action?.payload?.find(item2 =>item2.id === item.id).isLove || false,
              }
            }
          ),
        ],
